...
if __name__ == "__main__":
  ...
  start_move_number =  window_start_time * 1e15
  y       =  500
  x_end   =  60 * micron
  gridnumber = 1200
  ...
  xgrid   =  int(gridnumber + c*t_end/delta_x)
  ...
  for n in range(start,stop+step,step):
        ...
        if  n  <  start_move_number:
           ...
           for x in range(1,xgrid+1):
                   ...
                   bz[x][n-1]=data['Electric Field/Ey'].data[x][y]/blunt            
        else:
           for x in range(1,xgrid+1):
             #if x > 1200 :
                 if x-c*(time-window_start_time)/delta_x >= 0 and x-c*(time-window_start_time)/delta_x < gridnumber:
                   t.append(header['time']*1e15) #[fs]
   
                   bz[x][n-1]=data['Electric Field/Ey'].data[int(round(x-c*(time-window_start_time)/delta_x))][y]/blunt
                   #else:bz.append(0)
